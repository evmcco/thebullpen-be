...
class ZaboTransactions {
  ...
  static async upsertTransactions(username, accountId, transactions) {
    db.tx(t => {
      const queries = transactions.map(transaction => {
        return t.none("insert into zabo_transactions (id, username, account_id, transaction_json) values ($1, $2, $3, $4) on conflict (id) do nothing"
